...
def create_and_prepare_model(model_args, data_args, custom_args, lora_args):
    bnb_config = None
    quant_storage_stype = None
    ...
    if custom_args.use_unsloth:
        ...
        # Load model using standard AutoModelForCausalLM
        torch_dtype = (quant_storage_stype 
    ...
    peft_config = None
    if lora_args.use_peft_lora and not custom_args.use_unsloth:
        peft_config = LoraConfig(
            ...
            else lora_args.lora_target_modules,
        )
    ...
    if data_args.chat_template_format == "chatml":
        from utils import ChatmlSpecialTokens as SpecialTokens
        chat_template = DEFAULT_CHATML_CHAT_TEMPLATE
    elif data_args.chat_template_format == "zephyr":
        from utils import ZephyrSpecialTokens as SpecialTokens
        chat_template = DEFAULT_ZEPHYR_CHAT_TEMPLATE
    else:
        SpecialTokens = None
        chat_template = None

    if SpecialTokens is not None:
        tokenizer = AutoTokenizer.from_pretrained(
            model_args.model_name_or_path,
    ...
    if custom_args.use_unsloth:
        ...
        model = FastLanguageModel.get_peft_model(
            ...
            use_gradient_checkpointing=custom_args.gradient_checkpointing,
            random_state=custom_args.seed,
            max_seq_length=data_args.max_seq_length,
        )
